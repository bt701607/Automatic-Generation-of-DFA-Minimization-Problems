...
Looking at eq.~\ref{ch:4:eq:m(n)} we observe, there is a symbol $\sigma$ such that $(\delta(p,\sigma),\delta(q,\sigma)) \in m(n-1)$. Let $p',q'=\delta(p,\sigma),\delta(q,\sigma)$, so $(p',q')\in m(n-1)$.
...
\begin{proof}
	...
	\end{enumerate}
	This follows directly from while loop and its terminating condition of \CompDist\ (alg.~\ref{ch:2:m-minmark}, line 4--7). Given this, we will prove: There exists a shortest distinguishing word of length $n-1$ for some state pair, but there can be no longer such a word.
