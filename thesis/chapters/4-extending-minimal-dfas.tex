...
\noindent In order to fulfill these requirements we will deduce for equivalent and equivalent states how they may be added by examining their desired properties. If we get to have the choice between several equally good options, we will randomly select one.
...
Regarding the outgoing transitions of any $r_i$ equivalent to a state $e$, we are directly restricted by their equivalency relationship:
\begin{itemize}
	\item[] $r_i \sim_A e$
...
\end{itemize}
So we see, that the state $\delta(r_i, \sigma) = q$ must be in the same equivalency class as $\delta(e, \sigma) = p$. We may thus formulate the rule for adding outgoing transitions to a new state quite straightforward:
\begin{description}
