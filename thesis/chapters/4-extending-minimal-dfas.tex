...
\noindent In order to fulfill these requirements we will deduce for both kinds of states how they may be added by examining their desired properties. If we get to have the choice between several equally good options, we will randomly select one.
We will be guided by the separation of creating equivalent and unreachable states, thus distinct algorithms will be devised to create each kind of states. Concerning the planarity option, we again use a rejection algorithm with the same test as before (see sec.~\ref{ch:3:sec:planarity}). These decisions lead to the following main formulation of \textsc{ExtendMinDFA}:
\vspace{0.2cm}
\begin{spacing}{1}
	\begin{algorithmic}[1]
...
\end{spacing}
\vspace{0.2cm}
We will show for the action of adding equivalent states, that this does not change a DFAs $\mmD$-value - thus we do not have to care whether we are possibly changing it.
