...
\begin{definition}[Equivalent and Distinguishable State Pairs]\cite[p. 154]{HMU01}\label{ch:2:def:eq-dist-pairs}
	...
	\end{displaymath}
	If $q_0 \not\sim_A q_1$, then $q_0$ and $q_1$ are called a \emph{distinguishable} state pair. The relation $\sim_A$ is an equivalence relation. \gregor{Proof?}
\end{definition}
...
For example for an exam it would be sensible to be able to generate a task that is different in structure  , that has not been generated before. We will use the criterion of isomorphy to distinguish a possible solution DFA from all previously generated ones.
