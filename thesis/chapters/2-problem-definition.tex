...
%
%\begin{definition}[$\sim_A$-equivalence automaton] \label{ch:2:sim-eq-dfa}
...
%        \item $F_E = \{\ [q]\ |\ q \in Q\ \}$
%    \end{itemize}
%    Note, that we know by theorem~\ref{ch:2:min-alg-correct} that $E_A$ is guaranteed to be a minimal automaton to $A$, if $A$ has no unreachable states.
%\end{definition}
...
\begin{itemize}
	\item $\nSO$ is the number of states of $A_{sol}$
	\item $\nEQ$ is the number of distinct equivalent state pairs of $A_{re}$
	\item $\nUN$ is the number of unreachable states of $A_{task}$
\end{itemize}
...
The more symbols the alphabet of $A_{task}, A_{re}$ and $A_{sol}$ has (note that \MinAlg\ does not change the alphabet), the more transitions have to be followed when checking whether $(\delta(q,\sigma),\delta(p,\sigma))\in m(i-1)$ is true for each state pair $p,q$.
